...
function callChangeSide(e) {
  ...
  if (pressed == false && started && !dialog) {
    if (
      mpos.x >= pointer.x &&
      mpos.x <= pointer.x + pointer.d &&
      mpos.y >= pointer.y &&
      mpos.y <= pointer.y + pointer.d
    ) {
    ...
    for (let i in fallings) {
      if (
        mpos.x >= fallings[i].x &&
        mpos.x <= fallings[i].x + fallings[i].d &&
        mpos.y >= fallings[i].y &&
        mpos.y <= fallings[i].y + fallings[i].d
      ) {
        if (fallings[i].bad) {
          man.lose();
          fallings.shift();
  ...
  if (!started || dialog) {
    ...
    fallingInterval = setInterval(() => {
      fallings.push(new Falling(0, Math.floor(Math.random() * 2)));
    }, 5000);
