class Block {
  constructor(y) {
    this.w = (canvas.width-canvas.width/6*2-20) - (canvas.width/6*2+20);
  ...
  draw(){
    ...
    if(!this.passed){
      ...
      let color = animateColor(this.red, this.green, this.blue, 135, 132, 132, 0.2);
      this.red = color.r;
      this.green = color.g;
      this.blue = color.b;
    }
    ...
    ctx.fillText(this.value < 0.5 ? 'BUY' : 'SELL', this.x+this.w/2, this.y+this.h/2);
    ctx.restore();
  }
  ...
  move(){
    ...
    {
      if(!this.passed){
        if((man.side != 0 && this.value < 0.5) || (man.side != 1 && this.value >= 0.5)){
